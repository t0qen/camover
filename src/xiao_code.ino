...
void sendNotification(const char* title, const char* message, const char* topic, const char* priority) {
  ...
  String url = "https://ntfy.rpi-server.org/";
  url += topic;
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {          // attente wifi
    Serial.print(".");
    delay(500);
  }
...
void loop() {
  ...
  if (!client.connected()) {                       // verification mqtt

    if (millis() - last_mqtt_try > 5000) {         // tentative toutes les 5s
      last_mqtt_try = millis();
      Serial.println("connexion mqtt...");

      if (client.connect("camover")) {             // connexion broker
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  for (unsigned int i = 0; i < length; i++) {      // reconstruction chaine
  ...
  if (message == "OFF") {                          // extinction relais
